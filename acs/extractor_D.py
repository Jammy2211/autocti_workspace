...
"""
Load and plot the already complete ACS data reduction of this example image, for extraction in half D.
"""
file_path = path.join(dataset_path, "j9epf6kjq_rawD.fits")
...
print(f"Shape of Extraction D (HST reduction) = {image.shape_native}")

"""
Extraction D: Lets now use PyAutoArray's `ImageACS` class method to load quadrant D from this data, which does the 
following:

 - Loads the data from HDU 1.
 - Loads the right hand half of data corresponding to the numpy indexes array[0:2068, 2072:4144].

We will compare this to the already processed quadrant D dataset (which is in different units).
"""
...
print(f"Shape of Bias Extraction D (via autoarray) = {image_aa.shape_native}")
...
print(f"Shape of Bias Extraction D (via autoarray) = {residuals.shape_native}")
