...
frame = ac.acs.ImageACS.from_fits(
    file_path=f"{dataset_path}/jc0a01h8q_raw.fits", quadrant_letter="A"
)

bias_frame = ac.acs.FrameACS.from_fits(
    file_path=f"{dataset_path}/2581509lj_bia.fits", quadrant_letter="A"
)
...
plt.colorbar(im)
plt.axis("off")
plt.savefig(f"{output_path}/find_warm_pixels.png", dpi=400)
plt.close()
print(f"Saved {output_path}/find_warm_pixels.png")
...
warm_pixels.save(filename=f"{output_path}/{dataset_name}.pickle")
