...
This transdimensional pipeline runner fits `CIImaging` to calibrate parallel CTI where:
...
First, we create a `Setup`, which customizes:
 ...
 - The number of parallel `Trap` species and how they are modeled.
...
`no_parallel_traps` is True, the pipeline`s output paths are `tagged` with the string `parallel_traps_x2`.

This means you can run the same pipeline on the same data twice (e.g. with different numbers of parallel traps) and the 
...
The `path_prefix` below specifies the path the pipeline results are written to, which is:
 ...
 `autocti_workspace/output/transdimensional/parallel_x2`
"""
...
pipeline = parallel.make_pipeline(
    setup=None,
    settings=settings,
    total_parallel_traps=total_parallel_traps,
